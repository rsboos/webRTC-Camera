...
function createPeerConnection(fromId) {
  ...
  pc[fromId].onaddstream = handleRemoteStreamAdded;
  ...
  if (isInitiator) {
    try {
      ...
      sendChannel = pc[fromId].createDataChannel("sendDataChannel",
        {reliable: false});
    ...
    // connexion p2p
    pc[fromId].ondatachannel = gotReceiveChannel;
  }
}
...
function handleRemoteStreamAdded(event) {
 ...
 // reattachMediaStream(miniVideo, localVideo);
  attachMediaStream(remoteVideo[divRemoteVideo], event.stream);
